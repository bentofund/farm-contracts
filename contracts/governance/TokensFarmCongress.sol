...
contract TokensFarmCongress {
    ...
    modifier onlyMember {
        require(membersRegistry.isMember(msg.sender) == true, "Only TokensFarmCongress member can call this function");
        _;
    }
    ...
    external
    {
        require(address(membersRegistry) == address(0x0), "TokensFarmCongress:setMembersRegistry: membersRegistry is already set");
    ...
    payable
    {
        ...
        for (uint i = 0; i < proposal.targets.length; i++) {
            ...
            require(success, "TokensFarmCongress::executeTransaction: Transaction execution reverted.");
    ...
    function cancel(uint proposalId) external onlyMember {
        Proposal storage proposal = proposals[proposalId];
        // Require that proposal is not previously executed neither cancelled
        require(proposal.executed == false && proposal.canceled == false, "TokensFarmCongress:cancel: Proposal already executed or canceled");
    ...
    function _castVote(address voter, uint proposalId, bool support) internal {
        ...
        require(receipt.hasVoted == false, "TokensFarmCongress::_castVote: voter already voted");
